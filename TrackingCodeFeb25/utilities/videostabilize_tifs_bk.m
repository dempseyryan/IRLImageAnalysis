...
%startFrame = hiddenfiles +250;
framesOI = 175; %number of frames of interest
...
saveVid = 0; %save the whole sequence?
...
roih = rect(4);
getter.delete;
pointsA = detectSURFFeatures(leftI, 'MetricThreshold', ptThresh,'ROI',[roix roiy roiw roih],'NumOctaves',3,'NumScaleLevels',4);
pointsB = detectSURFFeatures(rightI, 'MetricThreshold', ptThresh,'ROI',[roix roiy roiw roih],'NumOctaves',3,'NumScaleLevels',4);
...
while ii < framesOI
   ...
   %visualize moving roi
%     figure(4);
%     imshow(imgA);
%     R1 = rectangle('Position',[roix roiy roiw roih], 'EdgeColor',[1 0 0]);
%     R2 = rectangle('Position',[troix troiy troiw troih],'EdgeColor',[0 1 0]);
    %%%
    
    HsRt = cvexTformToSRT(H);
  %  [H1,S1(ii),ANG1(ii),T1([(ii),(ii+1)]),R1] = cvexTformToSRT(H);
  %  Tx(ii) = Tx(ii-1) + H(3,1);         %%how am i collecting xy?
  %  Ty(ii) = Ty(ii-1) + H(3,2);
   % Tang(ii) = Tang(ii-1)+ANG1(ii);
    Hcumulative = HsRt * Hcumulative;
    HsRts= cat(3,HsRts,HsRt);
    Hcumulatives = cat(3,Hcumulatives,Hcumulative);
   % Txc(ii) = Hcumulative(3,1);
   % Tyc(ii) = Hcumulative(3,2);
   % Rc = Hcumulative(1:2,1:2);
   % TAngC(ii) = mean([atan2(Rc(2),Rc(1)) atan2(-Rc(3),Rc(4))]);
    imgBp = imwarp(imgB,affine2d(Hcumulative),'OutputView',imref2d(size(imgB)));
    %fused = imfuse(imgAp,imgBp,'blend');%,'ColorChannels','red-cyan'));
    
    if ~saveVid             %if not saving video, show images frame-by-frame
        figure(1);
        fused = imfuse(imgAp,imgBp,'blend');%,'ColorChannels','red-cyan'));
        imshow(fused);
