...
ARS1 = (ARS1-ARS1_offset)*pi/180;

ARS2 = -B*1e3 / 0.2000;
ARS2_offset = sum(ARS2(1:500))/500;
ARS2 = (ARS2-ARS2_offset)*pi/180;

ARS3 = -C*1e3 / -0.1955;
ARS3_offset = sum(ARS3(1:500))/500;
ARS3 = (ARS3-ARS3_offset)*pi/180;
...
csvwrite(sprintf('%slinearCSV.csv',path),[TCCy' ACC1y ACC2y ACC3y]);
csvwrite(sprintf('%srotationalCSV.csv',path),[TRSy' ARS1y ARS2y ARS3y]);
